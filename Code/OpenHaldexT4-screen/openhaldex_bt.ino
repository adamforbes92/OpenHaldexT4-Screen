...
bool btInit() {
  ...
  for (int i = 0; i < 12; i++) {
    ...
    if (i == 8) {
      uint16_t cMillis = millis();
      uint16_t sDelay = btTimeout * 3;

      while (millis() - cMillis < sDelay) {
        Serial.print(at_buf);
        ...
        if (p) {
          ...
          header = strtok(at_buf, ":");  // Parses "+CGNSINF: 1," from response
          addr1 = strtok(NULL, ":");     // Parses second CSV from SIM808 response (GPS fix value)
          addr2 = strtok(NULL, ":");
          addr3 = strtok(NULL, ",");
